...
from .forms import NewCategoryFeatureKeyForm, NewCategoryForm, NewProductForm
...
class CreateNewCategory(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_category.title = form.cleaned_data['title']
            new_category.save()
        return HttpResponseRedirect('/product-specs/')
...
class CreateFeatureView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.add_message(
            ...
            f'"{feature.feature_name}" в категории {category.title} успешно создано'
        )
...
class DeleteFromProductView(DeleteView):
...
class AttachNewFeatureToProduct(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        html_select = html_select.format(result=res_string)
        messages.add_message(request, messages.SUCCESS, f'Значения характеристик для товара {product.title} успешно добавлены')
            ...
            for item_for_update in data_for_update:
                if item.feature.feature_name == item_for_update['feature_name']:
                    if item.value != item_for_update['new_value'] and item_for_update['new_value'] != '---':
                        ...
                        item.save()
        messages.add_message(request, messages.SUCCESS, f'Значения характеристик для товара {product.title} успешно обновлены')
        return JsonResponse({"result": "ok"})
